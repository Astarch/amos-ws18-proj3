...
ssh -i AMOS_EC2_KEY.pem -o BatchMode=yes -o StrictHostKeyChecking=no ubuntu@18.216.129.153 "sudo kill $(ps aux | grep '/usr/lib/jvm/java-8-oracle/bin/java -classpath /usr/share/maven' | awk '{print $2}' || true) || true; sudo kill $(ps aux | grep 'sudo java -jar pretrendr-0.1.0' | awk '{print $2}' || true) || true; sudo kill $(ps aux | grep 'java -jar pretrendr-0.1.0' | awk '{print $2}' || true) || true; sudo rm -rf /home/ubuntu/pretrendr/backend/repo || true; sudo mv /home/ubuntu/backendRestDraft.tar.gz /home/ubuntu/pretrendr/backend; sudo rm -rf /home/ubuntu/backendRestDraft.tar.gz; cd /home/ubuntu/pretrendr/backend; sudo tar -xvzf backendRestDraft.tar.gz; cd /home/ubuntu/pretrendr/backend/repo; sudo rm /home/ubuntu/pretrendr/backend/backendRestDraft.tar.gz; cd /home/ubuntu/pretrendr/backend/repo/backendRestDraft; sudo sh run.sh </dev/null &>/dev/null &" 
